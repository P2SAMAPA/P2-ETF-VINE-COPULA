...
from vine_copula import vine_copula_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    if len(macro_window) < len(ret_window):
        return None
    raw_scores = vine_copula_score(ret_window, macro_window)
    norm_scores = normalize_scores(raw_scores)
    sorted_norm = sorted(norm_scores.items(), key=lambda x: x[1], reverse=True)
    top_etfs = [{"ticker": t, "vine_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/vine_copula_{timestamp}.json"
